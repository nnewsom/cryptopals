...
class RngCTR:
    ...
    def _gen_keystream(self,str_len):
        MT = MersenneTwister(self.seed)
        keystream = ""

        while str_len > len(keystream):
            tmp = MT.extract_number()
            keystream += chr( (tmp) & 0xFF)
            keystream += chr( (tmp >> 8) & 0xFF)
            keystream += chr( (tmp >> 16) & 0xFF)
            keystream += chr( (tmp >> 24) & 0xFF)
    ...
    def encrypt(self,plaintext):
        ...
        keystream = self._gen_keystream(len(plaintext))
    ...
    def decrypt(self,ciphertext):
        ...
        keystream = self._gen_keystream(len(ciphertext))
